...
export const DilevaryRes = () => {
  ...
  const dataSend = { user_id: userId, status: "delivery_request" };
  const getdelivarydata = () => {
    axios
      .post(
        "https://camp-coding.tech/ocean_burg/user/select_my_order_by_status.php",
  ...
  const columns = [
    {
      ...
      dataIndex: "name",
      key: "name",
      render: (_, record) => <space>{record.user_name}</space>,
    },
    {
      title: "email",
      key: "email",
    ...
    },
    {
      title: "company",
      key: "company",
      render: (_, record) => <space>{record.user_combany}</space>,
    },
    {
      title: "phone",
      key: "phone",
