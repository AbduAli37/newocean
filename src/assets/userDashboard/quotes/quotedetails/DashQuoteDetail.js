...
export const DashQuoteDetail = () => {
  ...
  const handleselect = (item,total_money,allrates) => {
    ...
    setTimeout(() => {
      setloading(false);
      navigate("/userdashboard/dashuserquoteorder", { state: { item: {...item,total_money,allrates} } });
    }, 3000);
  };
  ...
  const getimgssrc=()=>{
    ...
    for (let i = 0; i < flagdata.length; i++) {
      ...
      if (((Array.isArray(item.quotation_data.ports_destination_data)? item.quotation_data.ports_destination_data[0].country == flagdata[i].name:item.quotation_data.ports_destination_data.country == flagdata[i].name))||(item.quotation_data.ports_destination_data.province == flagdata[i].name)) {
                ...
                <div className="quotdet_rate" key={index}>
                  <div className="first_div">
                    <div>
                      ...
                      >
                        <span>{Array.isArray(item.quotation_data.ports_origin_data[0])?item.quotation_data.ports_origin_data[0]?.country:item.quotation_data.ports_origin_data?.country}</span>
                        <span><img style={{
                          width: "20px",
                        }} src={orimgsrc} alt="" /></span>
                        <span>{Array.isArray(item.quotation_data.ports_origin_data[0])?item.quotation_data.ports_origin_data[0]?.name:item.quotation_data.ports_origin_data?.name}</span>
                      </h4>
                      <h4>{`>>`}</h4>
                      <h4
                        style={{ display:'flex',alignItems:'center',justifyContent:'space-between',flexDirection:'column' }}
