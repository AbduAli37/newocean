...
export const DashQuoteOrder = () => {
  ...
  const [myalldata, setmyalldata] = useState({
    cotation_id: item.quotation_data.cotation_id,
    ...
    reat_id: item.quotation_data.rats_day_id,
  });
  ...
  const createorder = () => {
    ...
    axios
      ...
      .then((res) => {
        console.log(res.data.message,"ererer");
  ...
  const getimgssrc=()=>{
    ...
    for (let i = 0; i < flagdata.length; i++) {
      if ((Array.isArray(item.quotation_data.ports_origin_data)?item.quotation_data.ports_origin_data[0].country:item.quotation_data.ports_origin_data.country) == flagdata[i].name) {
      ...
      if ((Array.isArray(item.quotation_data.ports_destination_data)?item.quotation_data.ports_destination_data[0].country:item.quotation_data.ports_destination_data.country) == flagdata[i].name) {
      ...
      <div className="quote_order_details_div">
        <div className="quote_order_details">
          ...
          <div style={{ display:'flex',flexWrap:'wrap', }}>
                      {console.log(item.quotation_data.ports_origin_data)}
                      <h4
                        style={{ display:'flex',alignItems:'center',justifyContent:'space-between',flexDirection:'column' }}
                      >
                        <span>{Array.isArray(item.quotation_data.ports_origin_data)?item.quotation_data.ports_origin_data[0].country:item.quotation_data.ports_origin_data.country}</span>
                        <span><img style={{
                          width: "20px",
                        }} src={orimgsrc} alt="" /></span>
                        <span>{Array.isArray(item.quotation_data.ports_origin_data)?item.quotation_data.ports_origin_data[0]?.name:item.quotation_data.ports_origin_data.name}</span>
                      </h4>
                      <h4>{`>>`}</h4>
                      <h4
                        style={{ display:'flex',alignItems:'center',justifyContent:'space-between',flexDirection:'column' }}
                      >
                        <span>{Array.isArray(item.quotation_data.ports_destination_data)?item.quotation_data.ports_destination_data[0]?.country:item.quotation_data.ports_destination_data.country}</span>
                        <span><img style={{
                          width: "20px",
                        }} src={desimgsrc} alt="" /></span>
                        <span>{Array.isArray(item.quotation_data.ports_destination_data)?item.quotation_data.ports_destination_data[0]?.name:item.quotation_data.ports_destination_data.name}</span>
                      </h4>
                    </div>
          </div>
          <div className="quote_order_right">
            <div className="quote_order_time">
              ...
              {/* <div>
                ...
                <span>{item.quotation_data.estt}</span>
              </div> */}
            </div>
        ...
        <div className="quote_last_div">
          <div className="last_div_left">
            <h5>{item.quotation_data.cargo}</h5>
            {/* <h5>Fastest</h5>
            <h5>Cheapest</h5>
            <h5>direct</h5> */}
          </div>
        ...
        <div className="quote_order_service_right">
          <div>
            ...
            <div className="quote_order_service_origin">
              <h4>{Array.isArray(item.quotation_data.ports_origin_data)?item.quotation_data.ports_origin_data[0].name:item.quotation_data.ports_origin_data.name}</h4>
              ...
              cargetype=='chbrea'?(
                <>
                  <h4 className="total_money">
                    ...
                    <span>{item.quotation_data.total_money} USD</span>
                  </h4>
                  {
                ...
                <div className="quotedet">
                  ...
                  {/* <div>
                    <h4>origin port</h4>
                    <h4>
                      <span>{item.quotation_data.ready_date}</span>
                    </h4>
                  </div> */}
                  </div>
                  <div className="cargo_details">
                    ...
                    <h4>Shipment Type</h4>
                    <h4>
                      <span>{item.quotation_data.cargo}</span>
                    </h4>
                    </div>
                    {item.quotation_data.cargo=='lcl'?(
                      <div>
                        <h4>
                          Cargo Details
                        </h4>
                        <h4>
                          {item.quotation_data.lcl_data[0].height}*
                          {item.quotation_data.lcl_data[0].weight}*
                          {item.quotation_data.lcl_data[0].width}
                        </h4>
                      </div>
                    ):(
                      <div>
                        <h4>Shipment Type</h4>
                        <h4>
                        {item.quotation_data.fcl_data[0].qty}*
                        {item.quotation_data.fcl_data[0].container_type}
                      </h4>
                      </div>
                    )}
                  </div>
