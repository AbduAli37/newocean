...
const CountrieFlag = (props) => {
  const [countrieFlag, SetcountrieFlag] = useState("");

  useEffect(() => {
    flagdata.filter((data) =>
      data.name === props.name ? SetcountrieFlag(data) : null
    );
  }, [props.name]);
  const contrieCode = countrieFlag?.code;
  const image = require(`../../../../img/flags/${contrieCode && contrieCode != undefined ? contrieCode : 'AC'}.svg`);

  return (
    <React.Fragment>    
      <img src={image} alt={countrieFlag.name} />
    </React.Fragment>
  );
  // return <p>{countrieFlag.image}</p>;
};
